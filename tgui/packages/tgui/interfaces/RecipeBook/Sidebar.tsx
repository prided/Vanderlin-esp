  ...
  return (
    <Box style={{ width: '260px', height: '100%', borderRight: '2px solid var(--rb-border)', background: 'var(--rb-surface-alt)', display: 'flex', flexDirection: 'column' }}>
      <Box style={{ padding: '4px 6px', borderBottom: '1px solid var(--rb-border-faint)', flexShrink: 0 }}>
        <Input fluid placeholder="Buscar…" value={search} onChange={(value: string) => setSearch(value)} className="RecipeBook__search" />
      </Box>
      ...
      <Box overflowY="scroll" style={{ flex: 1 }}>
        {!filtered.length && <Box style={{ padding: '10px', color: 'var(--rb-text-muted)', fontStyle: 'italic', textAlign: 'center' }}>No hay recetas coincidentes.</Box>}
        {filtered.map((r, i) => (
