	...
	switch(action)
		...
		if("grant_language")
			if((is_admin || isobserver(AM)) && language_datum)
				...
				if(is_admin)
					message_admins("[key_name_admin(user)] otorgo el lenguaje [language_name] a [key_name_admin(AM)].")
		...
		if("remove_language")
			if((is_admin || isobserver(AM)) && language_datum)
				...
				if(is_admin)
					message_admins("[key_name_admin(user)] elimino el lenguaje [language_name] de [key_name_admin(AM)].")
