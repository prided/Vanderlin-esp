...
/datum/admins/proc/discord_manipulation()
	...
	switch(lookup_choice)
		if("ID")
			...
			if(returned_ckey)
				var/unlink_choice = tgui_alert(usr, "Discord ID [lookup_id] is linked to Ckey [returned_ckey]. Do you wish to unlink or cancel?", "Cuenta encontrada", list("Unlink", "Cancel"))
