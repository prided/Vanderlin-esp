...
/datum/culling_duel/New(mob/challenger, mob/target)
	...
	RegisterSignal(c_heart, COMSIG_QDELETING, PROC_REF(handle_challenger_heart_destroyed))
	RegisterSignal(t_heart, COMSIG_QDELETING, PROC_REF(handle_target_heart_destroyed))
...
/datum/culling_duel/Destroy()
	...
	if(d_challenger_heart)
		UnregisterSignal(d_challenger_heart, COMSIG_QDELETING)
	if(d_target_heart)
		UnregisterSignal(d_target_heart, COMSIG_QDELETING)
	return ..()
...
/datum/culling_duel/proc/finish_culling(mob/living/winner, mob/living/loser)
	...
	if(loser)
		...
		to_chat(loser, span_boldred("¡Has FALLADO Graggar por ULTIMA VEZ!"))
		loser.gib()
