/client/proc/jumptoarea()
	...
	message_admins("[key_name_admin(usr)] salto a [AREACOORD(T)]")
...
/client/proc/jumptomob()
	...
	if(T && isturf(T))
		...
		message_admins("[key_name_admin(usr)] salto a [ADMIN_LOOKUPFLW(M)] en [AREACOORD(M)]")
...
/client/proc/jumptokey()
	...
	log_admin("[key_name(usr)] jumped to [key_name(M)]")
	message_admins("[key_name_admin(usr)] salto a [ADMIN_LOOKUPFLW(M)]")
...
/client/proc/search_mob_dir()
	...
	var/mob/chosen_mob
	chosen_mob = tgui_input_list(usr, "Which mob are you looking for?", "Directorio de busqueda", GLOB.mob_directory)
	if(!chosen_mob)
		return
...
/client/proc/sendmob()
	...
	if(!length(sorted_areas))
		to_chat(src, "No se encontraron areas.")
		return
