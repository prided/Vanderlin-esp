...
/datum/quest/custom/freeform/build_from_user(mob/user)
	...
	var/obj_text = tgui_input_text(user, "Describe what the adventurer must do:", "Objetivo de la mision", "", 200)
	if(!obj_text)
		return FALSE
	...
	var/custom_title = tgui_input_text(user, "Dale un titulo a esta mision:", "Titulo de la mision", "", 80)
