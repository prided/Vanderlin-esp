...
/datum/rune_spell/bloodmagnetism/cast()
	...
	rejoin = tgui_alert(activator, "Will you pull them toward you, or pull yourself toward them?", "Magnetismo sanguineo", list("Summon Cultist", "Rejoin Cultist")) == "Rejoin Cultist"
	...
	var/choice = input(activator, "Choose who you wish to [rejoin ? "rejoin" : "summon"]", "Magnetismo sanguineo") as null|anything in annotated_targets
	if (!choice)
		qdel(src)
		return
...
/datum/rune_spell/bloodmagnetism/proc/success()
	if (target.occult_muted())
		for(var/mob/living/contributor in contributors)
			to_chat(activator, span_warning("El ritual fallo, el objetivo parece estar bajo una maldicion que nos impide alcanzarlo a traves del velo.") )
	else
		if (rejoin)
