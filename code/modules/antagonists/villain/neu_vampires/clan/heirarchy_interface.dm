	...
	switch(href_list["action"])
			...
			for(var/datum/clan_hierarchy_node/position in user_clan.all_positions)
				if(REF(position) == position_ref)
					if(can_manage_position(position))
						...
						to_chat(user, "<span class='warning'>No tienes permiso para modificar esta posicion.</span>")
					break
			refresh_hierarchy()
			...
			for(var/datum/clan_hierarchy_node/position in user_clan.all_positions)
				if(REF(position) == position_ref)
					if(can_manage_position(position) && position != user_clan.hierarchy_root)
						...
						to_chat(user, "<span class='warning'>No tienes permiso para eliminar esta posicion.</span>")
					break
			refresh_hierarchy()
		...
		if("edit_position")
			...
			if(target_position && can_manage_position(target_position))
				...
				to_chat(user, "<span class='warning'>No tienes permiso para editar esta posicion.</span>")
...
/datum/clan_hierarchy_interface/proc/handle_create_position(list/params)
	...
	if(!superior_position)
		to_chat(user, "<span class='warning'>Error: Posicion superior no valida</span>")
		return
	...
	if(new_position)
		...
		to_chat(user, "<span class='warning'>Error: No se pudo crear la posicion</span>")
...
/datum/clan_hierarchy_interface/proc/handle_assign_member(list/params)
	...
	if(!target_member)
		to_chat(user, "<span class='warning'>Error: Seleccion de miembro no valida</span>")
		return
	...
	if(target_member.clan_position && !can_manage_position(target_member.clan_position))
		to_chat(user, "<span class='warning'>Error: No tienes permiso para reasignar [target_member.real_name] desde su posicion actual</span>")
		return
