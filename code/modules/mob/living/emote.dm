...
/datum/emote/living/custom/can_run_emote(mob/user, status_check, intentional)
	...
	if(!isnull(user.ckey) && is_banned_from(user.ckey, "Emote"))
		to_chat(user, span_boldwarning("No puedes enviar emoticones personalizados (prohibidos)."))
		return FALSE
...
/datum/emote/living/hug/adjacentaction(mob/user, mob/target)
	...
	if(ishuman(target))
		...
		if(HAS_TRAIT(H, TRAIT_EORA_CURSE))
			to_chat(H, "<span class='warning'> ¡Me siento inexplicablemente repelido! </span>")
			H.cursed_freak_out()
			return
...
/datum/emote/living/kiss/adjacentaction(mob/user, mob/target)
	...
	if(ishuman(user) && ishuman(target))
		...
		if(HAS_TRAIT(E, TRAIT_EORA_CURSE))
			to_chat(E, "<span class='warning'> ¡Me siento inexplicablemente repelido! </span>")
			E.cursed_freak_out()
		...
		if(do_change)
			if(H.zone_selected == BODY_ZONE_PRECISE_MOUTH)
				...
				message_param = "besa %t en la frente."
			else
...
/datum/emote/living/snore
	...
	message = "snores."
	message_mime = "duerme profundamente."
	emote_type = EMOTE_AUDIBLE
	stat_allowed = UNCONSCIOUS
	snd_range = -4
