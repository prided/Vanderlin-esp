...
/datum/repeatable_crafting_recipe/engineering/automaton_frame
	...
	tool_usage = list(
		/obj/item/weapon/hammer = list(
			span_notice("comienza a martillar un marco de laton"),
...
/datum/orderless_slapcraft/automaton/step_process(mob/user, obj/item/attacking_item)
	if(istype(attacking_item, /obj/item/ingot/bronze))
		...
		user.visible_message(span_notice("[user] atornilla los sensores opticos en el craneo del marco."), span_notice("You screw the optical sensors into place. The lenses catch the light."))
...
/datum/orderless_slapcraft/automaton/try_finish(mob/user)
	...
	to_chat(user, span_notice("El automata esta completo. Espera un alma."))
