...
/datum/enchantment/on_hit/chaos_storm/apply_attack_effects(obj/item/source, mob/living/carbon/human/attacked, mob/living/carbon/attacker, actual_damage)
	switch(rand(1,5))
		if(1)
			...
			to_chat(attacked, span_warning("¡Llamas caóticas te envuelven!"))
		if(2)
			attacked.apply_damage(10, BRUTE)
			attacked.Knockdown(20)
			to_chat(attacked, span_warning("¡La fuerza caótica te golpea!"))
		if(3)
		...
		if(4)
			...
			to_chat(attacked, span_warning("¡La energía caótica altera tu coordinación!"))
		if(5)
			attacked.adjust_confusion(2 SECONDS)
			to_chat(attacked, span_warning("¡La energía caótica confunde tus pensamientos!"))
