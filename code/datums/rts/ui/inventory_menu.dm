...
/atom/movable/screen/worker_inventory_slot/proc/set_item(obj/item/item)
	...
	if(item)
		...
		name = "[parent_menu.get_slot_display_name(slot_type)]: Vacio"
...
/atom/movable/screen/worker_inventory_slot/proc/handle_right_click(mob/camera/strategy_controller/user)
	...
	var/choice = input(user, "Seleccione la ubicacion de almacenamiento:", "Store Gear") as null|anything in node_options
	if(!choice)
		return
