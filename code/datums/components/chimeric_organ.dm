...
/datum/component/chimeric_organ/proc/handle_node_injection(tier, purity, slot, datum/chimeric_node/injected_node, overlay_state)
	...
	if(!check_node_compatibility(injected_node))
		...
		if(organ_owner)
			to_chat(organ_owner, span_warning("El nodo rechaza el [parent]: ¡es incompatible con este tipo de organo!"))
		return
...
/datum/component/chimeric_organ/proc/ui_connect_nodes(input_id, output_id, mob/user)
	...
	if(organ_owner)
		to_chat(organ_owner, span_notice("Sientes que los humores en tu [parent] se alinean con un clic humedo."))
	return TRUE
