...
/mob/proc/create_tracker()
	RETURN_TYPE(/datum/component/team_monitor)
	if(!monitor_key)
		monitor_key = "monitor_[REF(src)]"
	var/tracking_beacon = AddComponent(/datum/component/tracking_beacon, monitor_key, null, null, TRUE, "#f3d594")
	var/datum/component/team_monitor/monitor = AddComponent(/datum/component/team_monitor, monitor_key, null, tracking_beacon)
	monitor.show_hud(src)
	return monitor

/mob/proc/return_tracker()
	RETURN_TYPE(/datum/component/team_monitor)
	return GetComponent(/datum/component/team_monitor)
