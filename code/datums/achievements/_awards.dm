...
/datum/award/proc/inform_user(mob/user)
	...
	switch(achievement_quality)
		if(ACHIEVEMENT_RARE)
			if(unlock_message)
				unlock_text = span_achievementrare("Logro desbloqueado:\n<b>[name]</b>")
			if(unlock_sound)
				unlock_sound_file = 'sound/achievement/achievement_rare.wav'
		if(ACHIEVEMENT_GOOD)
			if(unlock_message)
				unlock_text = span_achievementgood("Logro desbloqueado:\n<b>[name]</b>")
			if(unlock_sound)
				unlock_sound_file = 'sound/achievement/achievement_good.wav'
		if(ACHIEVEMENT_NEUTRAL)
			if(unlock_message)
				unlock_text = span_achievementneutral("Logro desbloqueado:\n<b>[name]</b>")
			if(unlock_sound)
				unlock_sound_file = 'sound/achievement/achievement_neutral.wav'
		if(ACHIEVEMENT_BAD)
			if(unlock_message)
				unlock_text = span_achievementbad("Logro desbloqueado:\n<b>[name]</b>")
			if(unlock_sound)
