...
/obj/structure/chem_separator/proc/can_process(turf/location)
	if(!burning)
		return FALSE
	if(273.15 + location.return_temperature() > required_temp) // Too hot to condense
		return FALSE
	if(!beaker)
		return FALSE
	if(beaker.reagents.total_volume >= beaker.reagents.maximum_volume)
...
/obj/structure/chem_separator/ui_data(mob/user)
	...
	data["temperature"] = reagents.total_volume ? reagents.chem_temp - T0C : location.return_temperature() // Thermometer is in Celsius
