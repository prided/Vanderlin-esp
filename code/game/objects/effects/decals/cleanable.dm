...
/obj/effect/decal/cleanable/attackby(obj/item/W, mob/user, list/modifiers)
	...
	if(istype(W, /obj/item/reagent_containers/glass))
		if(src.reagents && W.reagents)
			...
			if(!src.reagents.total_volume)
				to_chat(user, "<span class='notice'>[src] no es lo suficientemente gruesa para recoger!</span>")
				return
			if(W.reagents.total_volume >= W.reagents.maximum_volume)
				to_chat(user, "¡<span class='notice'>[W] esta lleno!</span>")
				return
			to_chat(user, "<span class='notice'>¡Recojo [src] en [W]!</span>")
	...
	if(W.get_temperature()) //todo: make heating a reagent holder proc
		if(istype(W, /obj/item/clothing/face/cigarette))
			...
			to_chat(user, "<span class='notice'>Caliento [name] con [W]!</span>")
	else
		return ..()
