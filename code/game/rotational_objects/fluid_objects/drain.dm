...
/obj/structure/fluid_drain/Initialize()
	...
	if(connected_pipe)
		...
		active = TRUE
		visible_message(span_notice("[src] se conecta al sistema de tuberias y comienza a funcionar."))
	else
...
/obj/structure/fluid_drain/process()
	...
	if(collected_fluids.total_volume > 0)
		...
		if(current_providing_reagent)
			...
			visible_message(span_notice("[src] se queda sin liquido para suministrar."))
...
/obj/structure/fluid_drain/proc/provide_fluids_to_pipe()
	...
	if(current_providing_reagent != highest_reagent.type)
		...
		visible_message(span_notice("[src] comienza a proporcionar [initial(highest_reagent.name)] al sistema de tuberias."))
...
/obj/structure/fluid_drain/examine(mob/user)
	...
	if(connected_pipe)
		. += span_info("Conectado a un sistema de tuberias de agua.")
	...
	if(active)
		. += span_notice("El desagüe recoge activamente liquidos del suelo.")
	else
